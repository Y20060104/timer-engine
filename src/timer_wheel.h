...
public:
  TimerWheel(Alloc alloc=Alloc{}) : slots_l1{}, slots_l2{}, slots_l3{}, current_tick_(0),alloc_(std::move(alloc)) {}
  ~TimerWheel();
...
template<typename Alloc>
void TimerWheel<Alloc>::insert(Timer** slots,int slot,Timer* t){
   t->next_=slots[slot];
   t->prev_=nullptr;
...
template<typename Alloc>
void TimerWheel<Alloc>::cancel(Timer *t) { t->cancelled_ = true; }

template<typename Alloc>
void TimerWheel<Alloc>::tick() {
  current_tick_ += 1;
...
template<typename Alloc>
void TimerWheel<Alloc>::cascade(int level,int slot)
{
  switch (level)
  {
  case 2:
