...
template<size_t N>
void TimerPool<N>::deallocate(Timer* t){
    t->prev_=nullptr;
    t->next_=free_list;
    free_list=t;
}
...
template<size_t N>
TimerPool<N>::TimerPool(TimerPool&& other) noexcept : pool_(other.pool_), free_list(other.free_list) {
    other.pool_ = nullptr;
